...
for (let i = 1; i <= 10; i++) {
    const tempIArr: number[][][][][] = [];
    for (let j = 1; j <= 10; j++) {
        const tempJArr: number[][][][] = [];
        for (let k = 0; k <= winningScore; k++) {
            const tempKArr: number[][][] = [];
...
function movePlayer(n: number, p: number) {
    p += n - 1;
    p %= 10;
    p += 1;
    return p;
...
function solve(p1: number, p2: number, p1ToWin: number, p2ToWin: number, starting: number) {
    if (p1ToWin <= 0) return [1, 0];
    if (p2ToWin <= 0) return [0, 1];
