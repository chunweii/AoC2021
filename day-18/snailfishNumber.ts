export class SnailfishNumber {
    public head: SnailfishNumber | number[];
    public tail: SnailfishNumber | number[];
    private parent?: SnailfishNumber;
    private isHeadOfParent?: boolean;
    private constructor(head: SnailfishNumber | number[], tail: SnailfishNumber | number[]) {
        this.head = head;
        this.tail = tail;
    }
    static createSnailfishNumber(head: SnailfishNumber | number, tail: SnailfishNumber | number) : SnailfishNumber {
    ...
    explode() {
        ...
        // left
        let availableSnailfish: SnailfishNumber | undefined = this.parent;
    ...
    split(head: boolean) {
        if (head) {
            if (this.head instanceof SnailfishNumber) throw new Error("cannot split non simple number");
            ...
            if (this.tail instanceof SnailfishNumber) throw new Error("cannot split non simple number");
    ...
    toString(): string {
        let str = "[";
        if (this.head instanceof SnailfishNumber) str += this.head.toString();
        else str += this.head[0];
        str += ",";
        if (this.tail instanceof SnailfishNumber) str += this.tail.toString();
        else str += this.tail[0];
        str += "]";
        return str;
    }
    getMagnitude():number {
